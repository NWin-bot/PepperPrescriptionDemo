...
#Loading of dieases content from csv.
def load_diseases():
...
def delete_from_history_all():
    ...
    #Deletion of locally stored user submissions.
    user_folder = os.path.join(app.config['UPLOAD_FOLDER'], current_user.email)
    hold = os.listdir(user_folder)

    for images in hold:
        if images.endswith(".jpeg") | images.endswith(".jpg") :
         os.remove(os.path.join(user_folder, images))
...
def go_to_profile_deleteaccount():
    #Deletion of locally stored user content.
    user_folder = os.path.join(app.config['UPLOAD_FOLDER'], current_user.email)
